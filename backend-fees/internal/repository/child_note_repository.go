...
func (r *PostgresChildNoteRepository) Delete(ctx context.Context, id uuid.UUID) error {
	result, err := conn(ctx, r.db).ExecContext(ctx, `DELETE FROM fees.child_notes WHERE id = $1`, id)
	if err != nil {
		return err
	}
	if rows, _ := result.RowsAffected(); rows == 0 {
...
func (r *PostgresChildNoteRepository) GetByID(ctx context.Context, id uuid.UUID) (*domain.ChildNote, error) {
	...
	err := conn(ctx, r.db).GetContext(ctx, &note, fmt.Sprintf(`
...
func (r *PostgresChildNoteRepository) ListAll(ctx context.Context, offset, limit int) ([]domain.ChildNote, int64, error) {
	var total int64
	err := conn(ctx, r.db).GetContext(ctx, &total, `SELECT COUNT(*) FROM fees.child_notes`)
	if err != nil {
		return nil, 0, err
	}
	...
	err = conn(ctx, r.db).SelectContext(ctx, &results, `
