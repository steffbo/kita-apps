...
- Care-hours resolution for Platzgeld was corrected on 2026-07-27. `fees.children.care_hours` only mirrors the period that is effective today, so it is `NULL` for children whose care-hours period starts in the future (for example entry date `2026-08-01`). Both `FeeService.Generate` and `FeeService.calculateChildcareFeeForChild` now call `FeeService.resolveCareHours`, which reads `fees.child_care_hours_history` and picks the period effective in the billed month, otherwise the closest upcoming period, then `children.care_hours`, and only as last resort the `DefaultCareHours` fallback of 45. The pure helper `careHoursAt` is unit-tested in `backend-fees/internal/service/care_hours_resolution_test.go`.
