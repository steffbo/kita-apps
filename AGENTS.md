...
- Einstufung income calculation was corrected on 2026-06-06 based on the 2025 Frankfurt (Oder) recommendation for Kita parent contributions and § 10 BEEG. `Basiselterngeld` is fee-relevant only above 3,600 EUR/year, `Elterngeld Plus` only above 1,800 EUR/year, and `Mutterschaftsgeld` is fee-relevant. The income JSON now also breaks out `minijobIncome`, `unemploymentBenefit`, `capitalIncome`, and `rentalIncome`; `otherIncome` remains for residual other income. Existing stored JSON without these fields reads as zero.
