...
- Care-hours resolution for Platzgeld was corrected on 2026-07-27. Both `FeeService.Generate` and `FeeService.calculateChildcareFeeForChild` call `FeeService.ResolveCareHours`, which reads `fees.child_care_hours_history` and picks the period effective in the billed month, otherwise the closest upcoming period, and only as last resort the `DefaultCareHours` fallback of 45. The pure helper `careHoursAt` is unit-tested in `backend-fees/internal/service/care_hours_resolution_test.go`.
- Child hours were de-normalized until 2026-07-27 and are now stored once. Migration `backend-fees/migrations/000028_drop_child_hours_columns.up.sql` backfills any column-only values into the history tables and drops `fees.children.care_hours`, `fees.children.legal_hours`, and `fees.children.legal_hours_until`. `fees.child_care_hours_history` and `fees.child_legal_hours_history` are the single source of truth; the down migration restores the columns from the currently effective periods.
- `domain.Child.CareHours`, `LegalHours`, and `LegalHoursUntil` are derived values. Read queries in `child_repository.go` and `household_repository.go` join `childCurrentHoursJoins` and select `childSelectColumns`, which resolve the period covering `CURRENT_DATE`. The API/JSON contract is unchanged, so no OpenAPI or frontend type regeneration was needed.
- Writes go to the history only: `Create` records the hours from the child's entry date, `Update` compares against the values effective today and writes a new period effective from today, and `UpsertCareHoursHistory`/`UpsertLegalHoursHistory` write explicit periods. The previous `syncCurrentCareHoursTx`/`syncCurrentLegalHoursTx` cache updates were removed, so hour values can no longer go stale when a period starts or ends without a write.
- The `hasWarnings` child filter checks whether any hours period exists at all instead of whether a period is effective today, so children who start in the future are not reported as missing master data.
- `repository.truncateDate` normalizes history dates to UTC midnight. Before that, `time.Now()`-based writes in a UTC+X timezone could store a period one day early.
- Stichtagsmeldung hour breakdowns (`loadHoursBreakdown`) read only from the history tables; children without a period covering the Stichtag are counted as unknown.
- Regression coverage: `backend-fees/internal/service/child_hours_history_integration_test.go` pins derived reads, future periods, fee resolution, and the warnings filter; the migration was verified up/down/up against a scratch PostgreSQL 16 database.
...
Current state:
- The child detail Platzgeld card no longer assumes 30 Std./Woche. `frontend/apps/beitraege/src/pages/ChildDetailPage.vue` resolves the effective care hours from `child.careHours` and, when that is empty because the child starts later, from the closest upcoming `care-hours-history` entry. Without any care hours it shows `Betreuungszeit nicht hinterlegt` instead of calculating a fee, and when a fee is shown the used basis (`Basis: 35 Std./Woche (ab 1.8.2026)`) is displayed. Before this fix, a child whose 35 h period started in the future showed the 30 h amount (277,60 €) while the Einstufung correctly showed 312,30 €.
- The Betreuungszeiten block on the same page shows `ab <Datum>: <Stunden>` instead of `Unbekannt` when only future Rechtsanspruch/Betreuungszeit periods exist. `careHours`/`legalHours` from the API always describe the period effective today.
- The Einstufungs-PDF was redesigned as a one-page document letter on 2026-07-27. `frontend/apps/beitraege/src/components/EinstufungPDF.vue` and `EinstufungPDF.css` now render a monochrome, serif Geschäftsbrief (Briefkopf with the Kita logo, Datumszeile, Betreff, Anrede, Angaben zum Kind, Beitragstabelle, Hinweiskasten, zweispaltiges Kleingedrucktes, Grußformel, kompakte Fußzeile) instead of the previous web-styled layout with colored boxes.
- The letterhead logo lives at `frontend/apps/beitraege/src/assets/knirpsenstadt-logo.png` (480 px, grayscale, ~35 KB). It is inlined as a data URL before printing, because the print popup is a blank document where relative asset URLs would not resolve; the absolute URL is used as fallback.
- Print geometry is tuned so that a single A4 page is used: `@page` margin `13mm 15mm 10mm`, `.page` width `180mm`, base font `9pt`. There is roughly 10 mm of slack, so spacing/font changes in `EinstufungPDF.css` must be re-verified against the one-page requirement (also for follow-up Einstufungen with four fee columns).
- The fee table is period-based instead of month-based as of 2026-07-27: each row is a Zeitraum (`Aug. 2026 – Feb. 2027`, `ab März 2027`, and for follow-ups the previous period marked `(bisher)`) with Bereich, Std./Woche, Platzgeld and Essensgeld. This avoids the earlier impression that months between the printed single-month columns were missing. There is no monthly total column; the yearly Vereinsbeitrag is shown as a note line below the table (either due with the first payment or, for follow-ups, already paid).
- Period boundaries come from `effectiveFromMonth`/`validFrom`, the optional `validUntil`, and the Krippe→Kindergarten transition month (first full month after the third birthday).
